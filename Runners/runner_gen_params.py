...
def run_task(task_kwargs, pbar=None):
    ...
    log_file_name = f"{dataset}_S{seed}_{generator}_AllScales_GenParams.log"
    log_file = os.path.join(LOG_DIR, log_file_name)
    
    with open(log_file, "w", encoding="utf-8") as f:
        f.write(f"Starting GenParams Tune: {dataset} - Seed {seed} - Gen {generator} - All Scales\n\n")
        f.flush()
    ...
    return f"[+] COMPLETED GenParams Tune: {dataset} (Seed {seed}, Gen {generator})"
