...
EXPERIMENT_SCRIPT = os.path.join(os.path.dirname(__file__), '..', 'experiment_full_tune_gen_params.py')
LOG_DIR = os.path.join(os.path.dirname(__file__), '..', 'logs', 'gen_params')
...
def build_tasks():
    ...
    tasks.reverse()
    tasks.sort(key=lambda x: x['seed'], reverse=True)
    return tasks
...
def run_task(task_kwargs, pbar=None):
    ...
    log_file_name = f"{dataset}_S{seed}_{generator}_F{fold}_AllScales_GenParams.log"
    log_file = os.path.join(LOG_DIR, log_file_name)
    
    with open(log_file, "w", encoding="utf-8") as f:
        f.write(f"Starting GenParams Tune: {dataset} - Seed {seed} - Gen {generator} - Fold {fold} - All Scales\n\n")
        f.flush()
    ...
    return f"[+] COMPLETED GenParams Tune: {dataset} (Seed {seed}, Gen {generator}, Fold {fold})"
