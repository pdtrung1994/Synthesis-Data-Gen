#!/bin/bash
#SBATCH --job-name=synth_gen_params
...
if [ -z "$SLURM_JOB_ID" ]; then
    ...
    mkdir -p Results
    RUN_NUM=1
    while [ -d "logs/gen_params_$(printf "%02d" $RUN_NUM)" ] || [ -d "Results/gen_params_$(printf "%02d" $RUN_NUM)" ]; do
        RUN_NUM=$((RUN_NUM + 1))
    done
    RUN_DIR_NAME="gen_params_$(printf "%02d" $RUN_NUM)"
    ...
    echo "================================================="
    echo "Calculating total tasks for GenParams Tune..."
    ...
    TOTAL_TASKS=$(${CMD} python3 Runners/runner_gen_params.py --get_task_count | tail -n 1)
...
echo "================================================="
echo "Starting GenParams Tune job on $(hostname)"
...
CMD="singularity exec myenvironment.simg"
SCRIPT="Runners/runner_gen_params.py"
